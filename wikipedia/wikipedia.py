...
USER_AGENT = f"wikipedia/{'.'.join(str(x) for x in __version__)} (https://github.com/goldsmith/Wikipedia/)"


def set_lang(prefix: str):
    """
    ...
    """
    global API_URL
    API_URL = f"https://{prefix.lower()}.wikipedia.org/w/api.php"
...
def set_rate_limiting(
    rate_limit: bool,
    min_wait: timedelta = timedelta(milliseconds=50),
):
    """
...
@cache
def search(
    query: str, results: int = 10, suggestion: bool = False
) -> Tuple[List[Generator[Any, None, None]], Union[str, None]]:
    """
    ...
    * radius - Search radius in meters. The value must be between 10 and 10000
    ...
    if "error" in raw_results:
        if raw_results["error"]["info"] in (
            ...
            raise HTTPTimeoutError(f"{latitude}|{longitude}")
        else:
            raise WikipediaException(raw_results["error"]["info"])
...
class WikipediaPage(object):
    ...
    def __repr__(self):
        return stdout_encode(f"<WikipediaPage '{self.title}'>")
    ...
    def __continued_query(self, query_params: Dict[str, Any]):
        """
    ...
    def revision_id(self):
        ...
        <https://en.wikipedia.org/wiki/Wikipedia:Revision>`_ for more
        information.
        """
...
def _wiki_request(params: Dict[str, Any]):
    """
